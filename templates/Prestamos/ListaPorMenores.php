...
if(!$auth){
    header('Location: /SystemLybrary/login.php');
        ...
        $fecha_menor = calcularMenor();

        //traigo todos los libros en prestamos vigentes para cierto socio
        $query = "SELECT * FROM detalleprestamo d JOIN prestamo p ON d.cod_prestamo = p.cod_prestamo JOIN libro l ON d.cod_libro = l.cod_libro JOIN socio s ON p.cod_socio = s.cod_socio WHERE p.fecha_devolucion IS NULL AND s.fnacimiento > '$fecha_menor'";
